...
function userid_details($id){
    $sql = "SELECT date,f_name,l_name,email,phone_no,username FROM users WHERE id_user=$id";
	$query = query_execute_sqli($sql);
	$row=mysqli_fetch_assoc($query);
	return $row;	
}
...
function update_member_wallet($user_id,$wallet_income,$inc_type)
{
	...
	if($inc_type == 1) { $income_type_log = "Daily Bonus"; }
	if($inc_type == 2) { $income_type_log = "Referral Income"; }
...
function get_paid_member($user_id){
	$sql = "SELECT t1.user_id,t2.id_user FROM reg_fees_structure t1
			...
			WHERE t1.user_id = '$user_id' and t1.mode in(1,2,0) and t2.type='B' and t1.update_fees > 0";
	$query = query_execute_sqli($sql);
	$result = mysqli_num_rows($query);
	mysqli_free_result($query);
	return $result;
...
function get_date_without_sat_sun($date,$day){
	
	for($i = 0; $i < $day; ){
		$d = date('D', strtotime($date."+1 DAY"));
		$date = date('Y-m-d', strtotime($date."+ 1 Day"));
		if($d != 'Sat' and $d != 'Sun'){
...
function all_country_detail(){
	$country_detail = array();
	$qu = query_execute_sqli("select * from country order by country_id asc ");
	$p = 0;
	while($r = mysqli_fetch_array($qu))
	{
		$country_detail[$p]['country_id'] = $r['country_id'];
		$country_detail[$p]['iso'] = $r['iso'];
		$country_detail[$p]['iso3'] = $r['iso3'];
		$country_detail[$p]['country_name'] = $r['country_name'];
		$country_detail[$p]['phonecode'] = $r['phonecode'];
		$p++;
	}
