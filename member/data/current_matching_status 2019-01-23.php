...
{ ?>
	<table class="table table-bordered table-hover">
		...
		while($row = mysqli_fetch_array($que))
		{
			...
			$cur_business = $rest_lp = $rest_rp = $tot_bus = $left_carry = $right_carry = $flush_business = 
			$max_pair = 0;
			
			$bin_date = date('Y-m-d' , strtotime(get_user_binary_qualifier_date($login_id)."- 1 Day"));
			...
			<tr class="text-center">
				...
				<td><?=$left_point1;?></td>
				<td><?=$right_point1;?></td>
				<td><?=$cf_left;?></td><!--<td><?=$rest_lp;?></td><td><?=$rest_rp;?></td>-->
				<td><?=$cf_right;?></td>
