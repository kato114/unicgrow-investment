...
$btn_submit = "<input type='submit' name='submit' value='Submit' class='btn btn-primary' />";
$tot_rental = get_user_which_type_bonus($login_id , 2);
$tot_binary = get_user_which_type_bonus($login_id , 4);
$pend_roi = $my_plan[5]-($tot_rental+$tot_binary);
$recvd_roi = $tot_rental+$tot_binary;
if(($recvd_roi) >= $my_plan[5]){
	$tot_roi = $total_roi;
	$btn_submit = "<B class='text-danger'>You have taken your fund !!</B>";
}
/*if($total_roi > 0){
	$tot_roi = $total_roi;
...
if(isset($_POST['approve'])){
	if($user_cancel_invst == '' or $user_cancel_invst == 2){	
		if($_SESSION['OTP_CAN_INV'] == $_POST['otp_code']){
			$sql = "INSERT INTO `cancel_investment`(`user_id`, `tot_roi`, `received_roi`,`received_bin`, `pending_roi`, 
			`req_date`, `paid_date`) 
			VALUES ('$login_id', '$tot_roi', '$tot_rental','$tot_binary', '$pend_roi', NOW(), DATE_ADD(NOW(), INTERVAL 45 DAY))";
...
if(isset($_POST['submit'])){
	...
	if($user_cancel_invst == '' or $user_cancel_invst == 2){ 
		...
		<table class="table table-bordered table-hover">
			...
			<tr><th colspan="2" class="text-danger">**Terms and Condition Applied</td></tr>
			<tr>	
	...
	else{ echo "<B class='text-danger'>Cancel Investment already Done By You!</B>"; }
}
?>
