...
while($row = mysqli_fetch_array($query)){
	...
	$pack_name = $plan_name[$row['package']-1];
	$pack_name = $pack_name == NULL ? "" : $pack_name; 
	$act_date = $row['act_date'];
	$type = $row['type'];
	$img = get_img($id_user,$type,$row['package']);
...
for($i = 0; $i < 4; $i++){
	...
	for($ps = 0; $ps < pow(2,($i+1)); $ps++){
		...
		$n_id = $pos_arr[$k];
		$m++;
		$sql = "SELECT t2.*,t3.username spons_id,COALESCE(t4.date,'0000-00-00') act_date,sum(t4.update_fees) invest_type FROM users t1
		...
		if($num > 0){
			while($row = mysqli_fetch_array($query))
			{
				...
				if($id_user == NULL or $id_user == "" or $id_user == 0){
					...
					$pack_name = $plan_name[$row['package']-1];
					$pack_name = $pack_name == NULL ? "" : $pack_name;
					$img = get_img($id_user,$type,$row['package'] > 0 ? $row['package'] : 0);
					$total_child = give_total_children($id_user);
