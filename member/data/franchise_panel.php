...
$user_id = $franch_id = $_SESSION['mlmproject_user_id'];
$franchise = franchise_information($user_id);
$location = $franchise[0];
$franchise_wallet = $franchise[1];
$qur_set_search = ' and t2.paid = 0 and t2.paid_by=1 and t1.location='.$location;
?>
<style>
.normal-button{
display:inline;
...
elseif(isset($_POST['paid']))
{
	...
	$amount = $_POST['amount'];
 $franchise_wallet;
	if($franchise_wallet >= $amount){
		$sql_update = "update daily_income set paid = 1, paid_by=".$location." where user_id = '$paid_id' and paid='0'	and paid_by=1";
		query_execute_sqli($sql_update);
		
		$sql_update = "update franchise set franchise_wallet = franchise_wallet-$amount where franchise_id =".$_SESSION['mlmproject_user_id']." and id=".$location;
		query_execute_sqli($sql_update);
		echo "<script type=\"text/javascript\">";
		echo "window.location = \"index.php?page=franchise_panel\"";
		echo "</script>";
		
		$franc_wall = frachise_wallet_bal($franch_id);
		insert_franch_wallet_account($franch_id, $amount, $systems_date, $fr_acount_type[2],$fr_acount_type_desc[2],2,$franc_wall);
	
	}
	else{
		echo "<strong><font color=\"red\"> Note :- You Don't Have Sufficent Balance Yet!! Permission Denied!!</font></strong>";
	}
}	
...
{	
	...
	$sql = "select * from(
			SELECT  t2.date as roi_date,t2.paid , 		
			 (sum(t2.income)-(sum(t2.income)*15)/100) as total_inc,t1.*
			 FROM users as t1 
	...
	if($totalrows > 0)
	{
		...
		if(isset($_REQUEST['roi_date']) and $_REQUEST['roi_date'] == 2)
		{
	...
	print "
		...
		<th colspan=9 align=\"right\" style=\"border:none;text-align: right;\">
			<form method=\"post\" action=\"index.php?page=franchise_panel\">
		...
		print "<tr><td colspan=9>&nbsp;</td></tr><td colspan=9 height=30px width=400 class=\"message tip\"><strong>";
			if ($newp>1)
			{ ?>
				<a href="<?php echo "index.php?page=franchise_panel&p=".($newp-1);?>">&laquo;</a>
			<?php 
			}
			for ($i=1; $i<=$pnums; $i++) 
			{ 
				if ($i!=$newp)
				{ ?>
					<a href="<?php echo "index.php?page=franchise_panel&p=$i";?>"><?php print_r("$i");?></a>
					<?php 
				}
				else
				{
			...
			if ($newp<$pnums) 
			{ ?>
			   <a href="<?php echo "index.php?page=franchise_panel&p=".($newp+1);?>">&raquo;</a>
			<?php 
			} 
			print"</strong></td></tr></table>";
	}
	else{ 
		print "There is no Income to Show !!&nbsp;&nbsp;<a href=\"index.php?page=franchise_panel\" class=\"normal-button\">Back</a>"; 
...
{
	$query_find = query_execute_sqli("SELECT id,franchise_wallet FROM franchise WHERE franchise_id = '$id'");
	$num = mysqli_num_rows($query_find);
	if($num != 0)
	{
		while($rows = mysqli_fetch_array($query_find))
