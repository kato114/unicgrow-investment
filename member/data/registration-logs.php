...
?>
<h1 align="left">Registration Logs</h1>
<?php
$newp = $_GET['p'];
$plimit = "15";

$user_id = $_SESSION['mlmproject_user_id'];
$query = query_execute_sqli("select * from logs where user_id = '$user_id' and type = '$log_type[2]' ");
...
if($totalrows != 0)
{
	...
	$query = query_execute_sqli("select * from logs where user_id = '$user_id' and type = '$log_type[2]' LIMIT $start,$plimit ");			
	...
	print "<tr><td colspan=4>&nbsp;</td></tr><td colspan=4 height=30px width=400 class=\"message tip\"><strong>";
		if ($newp>1)
		{ ?>
			<a href="<?php echo "index.php?page=registration-logs&p=".($newp-1);?>">&laquo;</a>
		<?php 
		}
		for ($i=1; $i<=$pnums; $i++) 
		{ 
			if ($i!=$newp)
			{ ?>
				<a href="<?php echo "index.php?page=registration-logs&p=$i";?>"><?php print_r("$i");?></a>
				<?php 
			}
			else
			{
		...
		if ($newp<$pnums) 
		{ ?>
		   <a href="<?php echo "index.php?page=registration-logs&p=".($newp+1);?>">&raquo;</a>
		<?php 
		} 
