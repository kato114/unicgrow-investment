...
$current_amount = get_user_allwallet($login_id,'activationw');
?>
<script>
...
$(document).ready(function() {	
	...
	$('.top_to').change(function() {
		...
		if(top_from == 10){
			...
			$("#community_row").show();
			$('#community_row').html("" );
			input = $('<th width="30%">Member User ID (<span id="user-search"></span>)</th><th><input type="text" name="requested_user" class="form-control" id="comm_id" value="" /></th>');
	...
	if(isset($_POST['submit'])){
		if($_POST['submit'] == 'OTP Valid'){
			if($_SESSION['WTDW_OTP'] == $_POST['valid_otp']){
				...
				echo '<label class="text-danger">Invalid OTP ...</label>';
		...
		if($_POST['submit'] == 'Request' or $_POST['submit'] == 'OTP Valid True'){
			...
			if(isset($_POST['requested_user'])){
				$request_user_id = get_new_user_id($_POST['requested_user']);
			}
			else{
			...
			$requested_user = $_POST['requested_user'];
			$req_user_id = get_new_user_id($requested_user);
			
			if($request_user_id == $login_id){ echo "<B class='text-danger'>You can't transfer to yourself !</B>"; }
			else{
				if($req_user_id < 1){ echo "<B class='text-danger'>Please Enter correct Username !</B>"; }
				else{
					$sql = "SELECT * FROM network_users WHERE FIND_IN_SET($req_user_id,left_network) 
					OR FIND_IN_SET($req_user_id,right_network) AND user_id = '$login_id'";
								...
								if($inc_chk == 1){ 
									if($request_amount >= $minimum_transfer_amt){
										if($_POST['submit']  == 'OTP Valid True'){ //=='Request'
											...
											<input type="hidden" name="req_user_id" value="<?=$req_user_id?>" />
											<table class="table table-bordered table-hover">
												<thead><tr><th colspan="2">Your Transaction Information</th></tr></thead>
												<tr>
													<th width="40%">Comapany Wallet Balance</th>
													<td>&#36;<?=round($current_amount,2)?></td>
												</tr>
												<tr><th>Transfer Amount</th><td>&#36;<?=$request_amount?> </td></tr>
												<tr><th>Receiver Id</th><td><?=$requested_user?></td></tr>
												<tr>
													<th>Receiver Name</th><td><?=get_full_name($req_user_id)?></td>
												</tr>
												<tr>
											...
											if($_POST['submit'] == 'Request'){
												if(!isset($_SESSION['WTDW_OTP'])){
													...
													if(strtoupper($soft_chk) == "LIVE"){
														...
														$message_login = "Your Transfer Cash-wallet OTP is ".$rand." https://www.unicgrow.com";
											...
											<form method="post" action="" enctype="multipart/form-data">
												...
												<div class="alert alert-success">
													<B>OTP Sent On Your Registered Mobile !!</B>
												</div>
												<div class="col-md-2"><B>Enter OTP</B></div>
												<div class="col-md-5">
													<input type="text" name="valid_otp" value="<?=$_POST['valid_otp']?>" class="form-control" />
												</div>
										...
										echo "<B class='text-danger'>Transfer minimum $minimum_transfer_amt &#36; amount !!</B>"; 
									}	
								}
								else{ 
					...
					else{ echo "<B class='text-danger'>Requested Member Not In Your Network !</B>"; }
				}
			}
		}
		elseif($_POST['submit'] == 'Submit' and $_SESSION['ses'] == 1){
			...
			if($_SESSION['request_amount'] == $req_amount){
				...
				$reqID_username = get_user_name($login_id);
				
				$wal_bal = get_user_allwallet($login_id,'activationw');
				insert_wallet_account($login_id, $req_user_id, $req_amount, $systems_date_time, $acount_type[42],$acount_type_desc[42], 2, $wal_bal,$wallet_type[2], $remarks);
				
				$wal_bal = get_user_allwallet($req_user_id,'activationw');
				insert_wallet_account($req_user_id, $login_id, $req_amount, $systems_date_time, $acount_type[43],$acount_type_desc[43], 1, $wal_bal,$wallet_type[2],$remarks);
		...
		if($num == 0 or $transfer_count < $max_transfer_count){ ?> 
			...
			<form name="money" action="" method="post">
			<table class="table table-bordered table-hover">
				<tr><th width="40%">Company Wallet Balance</th><td>&#36;<?=round($current_amount,2)?></td></tr>
				<tr><th>Minimum Transfer Amount</th><td>&#36;<?=$minimum_transfer_amt?></td></tr>
				<!--<tr id="community_row"></tr>-->
				<tr>
					<th width="30%">Member User ID <span id="user-search"></span></th>
					<td><input type="text" name="requested_user" class="form-control" id="comm_id" /></td>
				</tr>
				<tr>
					<th>Amount</th>
