	...
	<div class="panel-heading">
		<div class="panel-options">
			<ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#tab-1"><h3>Buy</h3></a></li>
				<li class=""><a data-toggle="tab" href="#tab-2"><h3>Sale</h3></a></li>
			</ul>
		</div>
	</div>
		...
		<div class="tab-content">
			<div id="tab-1" class="tab-pane active">
				<table class="table table-bordered table-hover">
					<thead>
						...
						<th class="text-center">Bid Unit Amount</th>
						<th class="text-center">Trade Unit Amount</th>
						<th class="text-center">Bid Total Amount</th>
						<th class="text-center">Trade Total Amount</th>
						<th class="text-center">&nbsp;</th>
					</thead>
					<?php
					$s_no = 1;
					$sql = "SELECT t1.id, t1.unit_amount bid_unit,t1.share bid_share,t1.date bid_date, COALESCE(t2.tx_unit,0) trade_unit,COALESCE(t2.share,0) trade_share,COALESCE(t2.date,'') trade_date,t1.mode
					FROM trade_buy t1
					LEFT JOIN trade_trasaction t2 ON t1.id = t2.buy_id
					WHERE t1.user_id='$login_id' and t1.type=1 $qur_set_search 
					ORDER BY t1.id DESC";
					$que = query_execute_sqli($sql);
					while($row = mysqli_fetch_array($que)){
						$share = $row["trade_share"] > 0 ? $row["trade_share"] : $row["bid_share"];
						switch($row['mode']){
							case 0 : $status = 
						...
						<tr class="text-center">
							...
							<td><?=$share?></td>
							<td>&#36;<?=$row["bid_unit"];?></td>
							<td>&#36;<?=$row["trade_unit"];?></td>
							<td>&#36;<?=$row["bid_unit"]*$share;?></td>
							<td>&#36;<?=$row["trade_unit"]*$share;?></td>
							<td><?=$status?></td>
						</tr> <?php
						$s_no++;
					} ?>
			...
			<div id="tab-2" class="tab-pane">
				<table class="table table-bordered table-hover">
					...
					<?php
					$s_no = 1;
					$sql = "SELECT t1.id, t1.unit_amount bid_unit,t1.share bid_share,t1.date bid_date,COALESCE(t2.tx_unit,0)
					 trade_unit,COALESCE(t2.share,0) trade_share,COALESCE(t2.date,'') trade_date,t1.mode
					FROM trade_buy t1
					LEFT JOIN trade_trasaction t2 ON t1.id = t2.sale_id
					WHERE t1.user_id='$login_id' and t1.type=2 $qur_set_search 
					ORDER BY t1.id DESC";
					$que = query_execute_sqli($sql);
					while($row = mysqli_fetch_array($que)){
						$share = $row["trade_share"] > 0 ? $row["trade_share"] : $row["bid_share"];
