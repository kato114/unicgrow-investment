	...
	<form name="invest" method="post" action="">
		<table class="table table-bordered table-hover">
			...
			<tr> 
				...
				<td> 
					<input type="text" name="deposit" value="" class="form-control" required />
				</td>      
			</tr> 
			<tr>     
				...
				<script>
					$(document).ready(function(){
						/*$('.slectOne').click(function() {
							$("#pay_method").hide();
							var pay_val = $("input[name='payment_method']:checked").val();
							if(pay_val == 5){
								$("#pay_method").show();
								var r = $('<input type="file" name="receipt" /> ');
								$("#pay_file").append(r); 
							}
							if(id_usd == 1){ $("#pay_method").hide(); }
						});*/
						$("#pay_method").hide();
					...
					<div class="form-group">
						...
						<?php
						$i = 1;
						$qu = query_execute_sqli("select * from payment_method where id in(1,5)");
						while($row = mysqli_fetch_array($qu))
						{
							$paym_id = $row['id'];
							...
							?>
							<div class="col-md-6">
								<input type="checkbox" name="payment_method" value="<?=$paym_id?>"  class="slectOne" data-id="<?=$i;?> selected" <?=$sel?> id="">&nbsp;<?=$currency_p?>
							</div>
