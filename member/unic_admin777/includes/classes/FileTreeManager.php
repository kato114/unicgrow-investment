...
class FileTreeManager implements ITreeManager 
{
	...
	{		
		$realreal_parent = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $real_parent);
		$fullPath = $this->getFullPath($realreal_parent. self::DEFAULT_FOLDER_SEPARATOR . $name);
		...
		if (file_exists($fullPath) === true) {
			...
			if ($slave == 1) {
				if (touch($fullPath) === true) {							
					$out = '({ "elementId":"'. $real_parent . self::FOLDER_SEPARATOR . $name . '", "elementName":"'.$name.'"})';
				}			
			}
			else{
	...
	{
		$realreal_parent = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $real_parent);
		$fullPath = $this->getFullPath($realreal_parent);
		$str = null;
		...
		if (is_dir($fullPath) && $handle = opendir($fullPath))
		{
		   	...
		   	while (false !== ($file = readdir($handle))) 
			{
				...
				if (is_dir($fullPath . "/" . $file)) {
					...
					$supp = "<ul class='ajax'>"
								."<li id='". $real_parent. self::FOLDER_SEPARATOR . $file ."'>{url:".$pageName."?action=getElementList&real_parent=". $real_parent. self::FOLDER_SEPARATOR . $file ."}</li>"
							."</ul>";
				}
	
				$fileStr[] .= "<li class='text' id='". $real_parent. self::FOLDER_SEPARATOR . $file."'>"
								."<span>". $file ."</span>"
									. $supp
								."</li>";		       			
			}
	...
	{
		$real_parent = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $real_parent);
		
		$newElementId = $real_parent . self::DEFAULT_FOLDER_SEPARATOR . $name;
		
		$elementId = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $elementId);
		$realElementId = $real_parent 
 	...
 	public function deleteElement($elementId, &$index = 0, $real_parent)
	{
		...
		$elementId = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $elementId);
		$elementId = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $real_parent)
					. self::DEFAULT_FOLDER_SEPARATOR 
					. substr($elementId, strrpos($elementId, "/"), strlen($elementId));
	...
	{ 		
		$oldreal_parent = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $oldreal_parent);
		$elementId = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $elementId);
		
		$realElementId = $oldreal_parent . self::DEFAULT_FOLDER_SEPARATOR 
						 . substr($elementId, strrpos($elementId, "/"), strlen($elementId));
		
		$fullPath = $this->getFullPath($realElementId);		
		$elementName = substr($realElementId, strrpos($realElementId, "/")+1); // plus 1 not to get / character
		$realDestreal_parent = str_replace(self::FOLDER_SEPARATOR, self::DEFAULT_FOLDER_SEPARATOR, $destreal_parent);
		
		$newFullPath = $this->getFullPath($realDestreal_parent . "/". $elementName);
		
		$newElementId = $destreal_parent . self::FOLDER_SEPARATOR . $elementName;
		$out = FAILED;
