...
<div class="row">
	...
	<form method="post" action="index.php?page=withdrawal_balance_rrequest">
...
if(isset($_POST['submit']))
{
	...
	elseif($_POST['submit'] == 'Cancel All')
	{
		...
		if($query){
			...
			<script>
				...
				window.location = "index.php?page=withdrawal_balance_rrequest";
			</script> <?php
		}
	...
	elseif($_POST['submit'] == 'Pay Bank')
	{
		...
		<script>alert("Request Accepted!"); window.location = "index.php?page=withdrawal_balance_rrequest";</script> 
		<?php
	}
...
elseif(isset($_POST['create_file']))
{
	...
	$insert_rows.="Withdrawal Request Of ROI " ;
	$insert_rows.="\n";
	$insert_rows.=" Username \t Name \t Phone(%) \t Total Withdrawal(USD) \t TDS(USD) \t Admin Tax(USD) \t Net Payble(USD) \t  Date \t Beneficiery Name \t Bank Account No. \t Bank Name \t Branch \t IFSC Code" ;
	$insert_rows.="\n";
...
{
	$sql = "SELECT T1.*,T2.username,T2.phone_no,T2.f_name,T2.l_name, T2.nominee,T4.name beneficiery_name,T4.bank_ac,T4.bank bank_name,T4.ifsc ifsc_code,T4.branch
	FROM withdrawal_crown_wallet T1 
	INNER JOIN users T2 ON T1.user_id = T2.id_user
	LEFT JOIN kyc T4 on T1.user_id = T4.user_id 
	WHERE T1.status in(65,0) AND T1.request_crowd > 0 and ac_type=2
	$qur_set_search
	ORDER BY T1.date DESC";
	...
	{ ?>
		<table class="table table-bordered">
			...
			<tr>
				<th colspan="10">Total Payble Amount : <?=$tot_amt; ?> &#36;</th>
				<td colspan="2">
			...
			while($row = mysqli_fetch_array($query))
			{
				...
				$bank_info = "Benf. Name = ".$row['beneficiery_name']."<br>A/c No. = ".$row['bank_ac']." <br>Bank Name = ".$row['bank_name'].'<br />IFSC No. = '.$row['ifsc_code'];
