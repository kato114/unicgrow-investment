	...
	{	
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			else
			{
				...
				$sql_insert_account = "insert into account (`user_id` , `dr` , `date` , `account`) values ('$paid_id' , '$amount' , '$e_date' , 'ROI PAID') ";
		...
		$_SESSION['success_roi'] = 1;
		echo "<script type=\"text/javascript\">";
		echo "window.location = \"index.php?page=show_daily_income&success=1\"";
		echo "</script>";
...
if($_REQUEST['success'] == 1 and isset($_SESSION['success_roi']) and $_SESSION['success_roi'] == 1)
{
	print "<font color=\"green\">Roi Paid Successfully</font>";
	$_SESSION['success_roi'] = '';
	unset($_SESSION['success_roi']);
...
{
	if(isset($_POST['inc_date']) and $_POST['inc_date']!='')
	{
		$inc_date1 = $_POST['inc_date'];
	}
	else
	{
		$inc_date1 = $systems_date;
	}
	$paid_id = $_POST['paid_id'];
	$inc_date = $systems_date;
	$amount = $_POST['amount'];
...
if(isset($_REQUEST['trans_to_franch']))
{
	...
	$sql = "SELECT  t2.id as daily_id,t2.user_id,t2.date as roi_date,t2.paid , 		
...
print	"</select>
		...
		while($row = mysqli_fetch_array($query))
		{
			...
			<form method=\"post\">
					...
					<input type=\"hidden\" name=\"inc_date\" value=\"$end_date\">
		...
		print "<tr><td colspan=9>&nbsp;</td></tr><td colspan=9 height=30px width=400 class=\"message tip\"><strong>";
			if ($newp>1)
			{ ?>
				<a href="<?php echo "index.php?page=show_daily_income&p=".($newp-1).$select_field;?>">&laquo;</a>
			<?php 
			}
			for ($i=1; $i<=$pnums; $i++) 
			{ 
				if ($i!=$newp)
				{ ?>
					<a href="<?php echo "index.php?page=show_daily_income&p=$i".$select_field;?>"><?php print_r("$i");?></a>
					<?php 
				}
				else
				{
			...
			if ($newp<$pnums) 
			{ ?>
			   <a href="<?php echo "index.php?page=show_daily_income&p=".($newp+1).$select_field;?>">&raquo;</a>
			<?php 
			} 
