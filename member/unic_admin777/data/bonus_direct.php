...
WHERE t1.type = 3 $qur_set_search ORDER BY t1.date DESC";	

$SQL = "$sql LIMIT $tstart,$tot_p ";

$sqlk = "SELECT COUNT(t1.id) num FROM income t1 WHERE t1.type = 3 $qur_set_search ";
...
{ 
		...
		<tr>
			<td>
				<form action="index.php?page=excel_bonus" method="post">
					<input type="hidden" name="bonus_name" value="Direct Bonus" />
					<input type="hidden" name="inc_type" value="3" />
					<input type="hidden" name="url" value="bonus_direct" />
					<input type="submit" name="Excel" value="Download Excel" class="btn btn-warning" />
				</form>
			</td>
			<form method="post" action="index.php?page=bonus_direct">
			<td>
	...
	<table class="table table-bordered">
		<thead><tr><th colspan="6">Total Direct Bonus : <?=$total_amount; ?> &#36;</th></tr></thead>
		<tr>
