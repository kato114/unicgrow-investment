...
{ 
	...
	<table class="table table-bordered table-hover">
		...
		<tr>
			...
			<th class="text-center" rowspan="2">Lottery</th>
		</tr>
		<tr>	
		...
		while($row = mysqli_fetch_array($que))
		{
			...
			$bkl = $row['bkl'];
			$bkr = $row['bkr'];
			$flush_business = $row['flush_business']*10;
			...
			<tr class="text-center">
				...
				<td><?=$net_business?></td>
				<td><i class="fa fa-<?=week_lottery_exist($login_id,$date_p)? 'check': 'times';?>-circle fa-2x"></i></td>
			</tr> <?php
			$sr_no++;
		} ?>
	</table> <?php
