...
if(isset($_POST['update'])){
	...
	$sql = "UPDATE `video_marketing` SET `title`='$title', `video_link`='$video_link', `date` = NOW() 
	WHERE id='$table_id'";
	query_execute_sqli($sql);
	?> <script>alert("Video Successfully Edited!"); window.location = "index.php?page=<?=$val?>";</script> <?php
...
if(isset($_POST['delete_video'])){
	$table_id=$_POST['table_id'];
	query_execute_sqli("DELETE FROM `video_marketing` WHERE id = '$table_id'");
	query_execute_sqli("ALTER TABLE `video_marketing` DROP `id`");
	query_execute_sqli("ALTER TABLE `video_marketing` ADD `id` INT NOT NULL AUTO_INCREMENT FIRST , ADD PRIMARY KEY (id)");
			
	?> <script>alert("Video Successfully Deleted!"); window.location = "index.php?page=<?=$val?>";</script> <?php
}
elseif(isset($_POST['edit_video'])){
	$table_id=$_POST['table_id'];
	$sql = "SELECT * FROM video_marketing WHERE id = '$table_id'";
	...
	$sql = "SELECT * FROM video_marketing";
