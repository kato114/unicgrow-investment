		...
		{	
			...
			if($tot_bonus >= $setting_min_withdrawal)		
			{
				...
				// print $u_id."-".$left_amnt."-".$wall_bal.'-'.$tot_bonus.'-'.$setting_min_withdrawal;
					...
					//query_execute_sqli("update wallet set roi = roi-'$roi_bonus' where id = '$u_id' ");
...
{	if($query_Search == '')
	...
	$query_Search .= " and date <= '$month_last_date' ";
	
 	$sqli = "select last_tab.*, roi_b as tot_b from ( 
				select user_id$date_field, coalesce(roi_b, 0) as roi_b
				from(
						SELECT t1.user_id,
						sum(case when t1.type=2 then t1.amount end) as roi_b
						$find_interval
			...
			<tr align="right" height="50">
				<th class="text-center" colspan=12>
					<strong style="font-size:20px;">
						...
						<a href="index.php?page=roi_pay_excel">Click Here</a>
					</strong>
				</th>
			</tr>
