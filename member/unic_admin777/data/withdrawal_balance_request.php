...
<div class="row">
	...
	<form method="post" action="index.php?page=withdrawal_balance_request">
...
if(isset($_POST['submit']))
{
	if($_POST['submit'] == 'Accept')
	{
		...
		$sql = "UPDATE withdrawal_crown_wallet SET accept_date = '$systems_date_time' , status = 0 , 
	...
	elseif($_POST['submit'] == 'Pay Bank')
	{
		...
		<script>alert("Request Accepted!"); window.location = "index.php?page=withdrawal_balance_request";</script> 
		<?php
	}
	elseif($_POST['submit'] == 'Cancel')
	{
		...
		if($wall_type == 3)
		{ 
			$wal_field = 'level_incentive';
			$total_amount = get_user_match_wallet($u_id);
		}
		
	 	$tax_amount = 0;//($req_amount*100)/(100-$withdrwal_money_tax)-$req_amount;
		$bal_amount = $total_amount+$req_amount+$tax_amount;		
		...
		<script>
			...
			window.location = "index.php?page=withdrawal_balance_request";
		</script> <?php
	}
...
elseif(isset($_POST['create_file']))
{
	...
	$insert_rows.=" Username \t Name \t Phone(%) \t Wallet Amount \t Request Amount(USD) \t Payment Mode \t Withdrawal Type \t Date \t Beneficiery Name \t Bank Account No. \t Bank Name \t IFSC Code";
	$insert_rows.="\n";
...
{
	$sql = "SELECT T1.*,T2.username,T2.phone_no,T2.f_name,T2.l_name, T2.nominee
	FROM withdrawal_crown_wallet T1 
	INNER JOIN users T2 ON T1.user_id = T2.id_user
	LEFT JOIN kyc T4 on T1.user_id = T4.user_id 
	WHERE T1.status in(65,0) AND T1.request_crowd > 0 
	$qur_set_search
	ORDER BY T1.date DESC";
	...
	{ ?>
		<table class="table table-bordered">
			...
			<tr>
				<th colspan="11">Total Amount : <?=$tot_amt; ?> &#36;</th>
				<td colspan="2">
			...
			while($row = mysqli_fetch_array($query))
			{
				...
				$request_amount = $request_amount + $row['tax'];
				$request_date = date('d/m/Y', strtotime($row['date']));
				$wallet_type = $row['ac_type'];
				$bit_ac_no = $row['ac_no'];
				$user_comment = $row['user_comment'];
				
				$bank_info = "Benf. Name = ".$row['name']."<br>A/c No. = ".$row['bank_ac']." <br>Bank Name = ".$row['bank'].'<br />IFSC No. = '.$row['ifsc'];
				...
				<tr class="text-center">
					...
					<td>
						<input type="submit" name="submit" class="btn btn-info btn-xs" value="Pay Bank" /><br /><br />
