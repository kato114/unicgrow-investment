...
include("../function/setting.php");
	...
	//	$schema_insert_rows = "Transaction Type". "\t";
		...
		$schema_insert_rows.= "Direct Income\t";
		$schema_insert_rows.= "Binary Income\t";
		...
		//	$schema_insert = strtoupper("r")."\t";
			...
			$schema_insert .= strtoupper(($username).$sep);
			$schema_insert .= strtoupper(($name).$sep);
			$schema_insert .= strtoupper(($direct_bonus).$sep);
			$schema_insert .= strtoupper(($binary_bonus).$sep);
			$schema_insert .= strtoupper(($tot_bonus).$sep);
			$schema_insert .= strtoupper(($admin_tax_anount).$sep);
			$schema_insert .= strtoupper(($admin_tds_anount).$sep);
			$schema_insert .= strtoupper(($left_amnt).$sep);
			$schema_insert .= strtoupper(($start_date." To ".$end_date).$sep);
	...
	?>
		<p><a style="color:#333368; font-weight:600;" href="index.php?page=payment_report">Back</a></p>
