...
<?php
$sql = "SELECT t1.*,t2.username 'from',t2.f_name,t2.l_name,t3.username FROM income t1
left join users t2 on t1.incomed_id = t2.id_user
left join users t3 on t1.user_id = t3.id_user
WHERE t1.type = 3 $qur_set_search ORDER BY t1.date DESC";

$SQL = "$sql LIMIT $tstart,$tot_p ";

$sqlk = "SELECT SUM(amount/token_rate) amt, COUNT(t1.id) num FROM ($sql) t1";
...
if($totalrows > 0){ ?>
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<th class="text-center">By User ID </td>
		</tr>
		</thead>
		<?php
