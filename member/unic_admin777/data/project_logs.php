...
$query = query_execute_sqli("select * from logs ");
$totalrows = mysqli_num_rows($query);
if($totalrows != 0)
{
	print "<table align=\"center\" hspace = 0 cellspacing=0 cellpadding=0 border=0 width=800>
			<tr>Projects Logs <p></p><th  height=30px width=200 class=\"message tip\"><strong>Date</strong></th>
	...
	$query = query_execute_sqli("select * from logs LIMIT $start,$plimit ");			
	...
	print "<tr><td colspan=4>&nbsp;</td></tr><td colspan=4 height=30px width=400 class=\"message tip\"><strong>";
		if ($newp>1)
		{ ?>
			<a href="<?php echo "index.php?page=project_logs&p=".($newp-1);?>">&laquo;</a>
		<?php 
		}
		for ($i=1; $i<=$pnums; $i++) 
		{ 
			if ($i!=$newp)
			{ ?>
				<a href="<?php echo "index.php?page=project_logs&p=$i";?>"><?php print_r("$i");?></a>
				<?php 
			}
			else
			{
		...
		if ($newp<$pnums) 
		{ ?>
		   <a href="<?php echo "index.php?page=project_logs&p=".($newp+1);?>">&raquo;</a>
		<?php 
		} 
