...
if(isset($_POST['submit']))
{
					...
					<table class="table table-bordered">
						...
						<tr>
							<td colspan="2" class="text-center">
								<input type="submit" name="submit" value="Upgrade" class="btn btn-info" />
							</td>
						</tr> <?php
						} ?>
						</form>
	...
	elseif($_POST['submit'] == 'Upgrade')
	{
		...
		$sql = "select * from reg_fees_structure 
				where user_id='$user_id' order by id asc limit 1";
		...
		if($plan_id > 0 and $_SESSION['adm_upg'] == 1){
			$_SESSION['adm_upg'] = 0;
			...
			VALUES ('$user_id' , '$rcw_id' , '$pv' , '$inv_amount' , '$systems_date' , '$start_date', '$profit' , '$total_days' , '$plan_id', '$p_value' , '$systems_date_time','$pre_count','1','Plan Upgrade By Admin',$pos) ";
			...
			`date_time`) VALUES ('$user_id','$insert_id','Upgrade Member By Admin','0','$inv_amount',(SELECT amount FROM wallet where id='$user_id'), '$systems_date_time')";
			query_execute_sqli($sqk);
			$sql = "UPDATE reg_fees_structure SET mode = '98' WHERE id = '$table_id'";
			query_execute_sqli($sql);
			...
			window.location = 'index.php?page=upgrade_investment'</script> <?php
		}
		else{ ?> <script>window.location = "index.php?page=upgrade_investment";</script> <?php }
	}
}	
else
{
$_SESSION['adm_upg'] = NULL;
unset($_SESSION['adm_upg']); 
?> 
<form name="my_form" action="index.php?page=upgrade_investment" method="post">
<table class="table table-bordered">
