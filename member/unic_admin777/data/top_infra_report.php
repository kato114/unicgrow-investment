...
if(isset($_POST['Excel']))
{
	...
	$sql = "SELECT t1.*,t2.request_crowd tot_invst,t2.profit,t2.boost_id,t2.total_days,t2.mode reg_mode, 
	t2.update_fees,t3.remarks,t3.date block_date 
	FROM users t1 
	INNER JOIN (select * from reg_fees_structure where id in(select max(id) from reg_fees_structure group by user_id) )  t2 ON t1.id_user = t2.user_id
	INNER JOIN (select * from account where id in(select max(id) from account where type=28 group by user_id)) t3 on t1.id_user = t3.user_id
	WHERE t2.id IS NOT NULL $qur_set_search GROUP BY t2.user_id ORDER BY t1.id_user DESC";	
		...
		<table class="table table-bordered">
			...
			<tr>
				<th class="text-right" colspan="13">
					...
					<form method="post" action="simple_view_tir.php" target="_blank"> 
