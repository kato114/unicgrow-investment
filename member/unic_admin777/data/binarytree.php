...
if(count($_GET) == 1){
	unset($_SESSION['bck_frdadm']);
}
if(isset($_POST['Search'])){
	if($_POST['search_username'] != ''){
		$_SESSION['SESS_search_username'] = $search_username = $_POST['search_username'];
		$login_id = $id = get_new_user_id($search_username);
		unset($_SESSION['bck_frdadm']);
		$_SESSION['bck_frdadm'][] = $login_id;
	}
}
if(isset($_GET['btp'])){
	$request_user_id = geting_best_position($login_id,$_GET['btp'])[0];
...
if(isset($_GET['pl']) and count($_SESSION['bck_frdadm']) > 0){
	$pc = count($_SESSION['bck_frdadm']);
	$bck_frdadm = $_SESSION['bck_frdadm'][0] > 0 ? $_SESSION['bck_frdadm'][0] : $login_id;
	if($bck_frdadm != $login_id and $login_id < $bck_frdadm){
		$sql = "select * from users where id_user='$bck_frdadm'";
	...
	array_values($_SESSION['bck_frdadm']);
}
...
<style>
.tree-binary { position: relative;min-height:500px; }
...
<div class="col-lg-12">
	<div class="widget-area">
		...
		<div class="row  pull-right">
			<div class="col-md-4">
				<form action="index.php?page=spons_info" method="post" target="_blank">
					<input type="hidden" value="<?=$login_id?>" name="user_id" />
					<input type="submit" name="more" value="My Sponser" class="btn btn-success btn-sm" />
				</form>
			</div>
			<div class="col-md-4">
				<form action="index.php?page=current_matching_status" method="post" target="_blank">
					<input type="hidden" value="<?=$login_id?>" name="user_id" />
					<input type="submit" name="more" value="Binary History" class="btn btn-success btn-sm" />
				</form>
			</div>
			<div class="col-md-4">
				<form action="index.php?page=member_list" method="post" target="_blank">
					<input type="hidden" value="<?=$login_id?>" name="user_id" />
...
<div class="col-lg-4 text-center">
	 <img src="assets/img/a7.png" height="50"><br /><B>Available</B>
</div>
<!--<div class="col-lg-2 text-center">
	 <img src="assets/img/a6.png" height="50"><br /><B>Blank Position</B>
</div>

<div class="col-lg-2 text-center">
	 <img src="assets/img/a4.png" height="50"><br /><B>Registered ID</B>
</div>-->
<div class="col-lg-4 text-center">
	 <img src="assets/img/aa1.png" height="50"><br /><B>ETH SILVER</B>
</div>
<div class="col-lg-4 text-center">
	 <img src="assets/img/aa2.png" height="50"><br /><B>BTC GOLD</B>
</div>
<div class="col-lg-12">&nbsp;</div>
...
while($row = mysqli_fetch_array($query)){
	...
	$cur_lottery = get_cur_week_lottery($id_user,$systems_date);
...
function get_img($id,$type,$invest_type){
	...
	if($type == 'B' and $num != 0) { 
		...
		$imges = "assets/img/$img.png"; 
	}
	if($type == 'B' and $num == 0) { $imges = "assets/img/a4.png"; }
	elseif($type == 'C'){ $imges = "assets/img/1.png"; }
	elseif($type == 'D'){ $imges = "assets/img/a5.png"; }
	return $imges;
}
function get_img_ol($id,$type){
	$num = 0;
	if($id != "")$num = 1;
	if($type == 'B' and $num != 0) { $imges = "assets/img/f.png"; }
	if($type == 'B' and $num == 0) { $imges = "assets/img/p.png"; }
	if($type == 'C'){ $imges = "assets/img/disabled.png"; }
	if($type == 'D'){ $imges = "assets/img/disabled.png"; }
	return $imges;
}
?>
<script>
...
function drawBinaryTree(target, levels) {
	...
	var levels = (levels == null) ? 5 : levels;
	var maxElements = Math.pow(2, levels-1);
	...
	for (l = 1; l <= levels; l++) {
		...
		for (i = 1; i <= elementsInLevel; i++) {
			...
			element.append('<img class="tree-element-image" width="100%" src="assets/img/a6.png">');
			...
			if (emptyPos) {
				...
				if (positionUsers[parentLevel] != null) {
					if (positionUsers[parentLevel][parentPosition] != null) {
						...
						className = '.pos-'+l+'-'+i;
						element = $(className);
						element.html('<a href="#"><img src="assets/img/a7.png" width="100%"></a>');
...
function business_summary_cf($id_user,$ul_lps,$ur_lps,$step_topup,$p_date,$n_date){
	...
	return array($cleft_point,$cright_point,$LCF,$RCF);
}
?>