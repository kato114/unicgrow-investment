...
if(count($_GET) > 1){
	$_POST['Search'] = '1';
	$_POST['search_user_name'] = $_SESSION['SESS_search_user_name'];
	$_POST['st_date'] = $_SESSION['SESS_st_date'];
	$_POST['en_date'] = $_SESSION['SESS_en_date'];
}
else{
	unset($_SESSION['SESS_search_user_name'],$_SESSION['SESS_st_date'],$_SESSION['SESS_en_date']);
}
if(isset($_POST['Search'])){
	if($_POST['search_user_name'] !=''){
		$_SESSION['SESS_search_user_name'] = $search_user_name = $_POST['search_user_name'];
		$search_id = get_new_user_id($search_user_name);
...
<table class="table table-bordered">
	<tr>
		...
		<td><input type="text" name="search_user_name" placeholder="Search By User ID" class="form-control" /></td>
...
{
	$file_name = "Manual-Booster Report".date('Y-m-d').time();
	$sep = "\t";
	...
	WHERE t1.rcw_id = 100 $qur_set_search ORDER BY date DESC";
		...
		<table class="table table-bordered">
			...
			while($r = mysqli_fetch_array($quer))
			{
				...
				<tr class="text-center">
					...
					<td><?=$date?></td>
					<td>&#36;<?=$update_fees?></td>
					<td><B class="text-success">Booster</B></td>
				</tr> <?php
				$sr_no++;
			} ?>
		</table> <?php
